    ...
    if (internals->hasInput) { // If audio input is enabled.
        if (buffersAvailable * internals->buffersize >= internals->latencySamples) { // if we have enough audio input available
            if (!internals->callback(internals->clientdata, audioGoesToOutput, internals->buffersize, internals->samplerate)) {
                ...
                //SuperpoweredTimeStretching *timeStretch = new SuperpoweredTimeStretching(bufferPool, internal->samplerate);
                SuperpoweredTimeStretching *timeStretch = new SuperpoweredTimeStretching(audioGoesToOutput, internals->samplerate);
                timeStretch->setRateAndPitchShift(1.00f, 10); // Audio will be 4% faster. -> x. pitch shift to 10.(of 12)
...
int SuperpoweredAndroidAudioIO::setPitchCent(int pitchCent) {
    mPitchCent = pitchCent;
